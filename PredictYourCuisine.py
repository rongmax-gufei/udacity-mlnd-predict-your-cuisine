...
import numpy as np
sum_ingredients = np.sum(sum_ingredients)
...
italian_ingredients_array = []
italian_ingredients = train_content[train_content.cuisine == 'italian']['ingredients']
for italian_ingredient in italian_ingredients:
    italian_ingredients_array.extend(italian_ingredient)
italian_ingredients = Counter(italian_ingredients_array)
...
X_train , X_valid , y_train, y_valid = train_test_split(train_tfidf, train_targets, test_size=0.2, random_state=20)
